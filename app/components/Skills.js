      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-3xl mx-auto">
            Comprehensive expertise in modern web development technologies and cloud platforms
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 mb-6">
          {skillCategories.map((category, categoryIndex) => (
            <div key={categoryIndex} className="bg-white rounded-2xl p-6 shadow-lg transition-all duration-300 hover:shadow-xl border border-gray-100">
              ...
              <div className="grid grid-cols-3 gap-4">
                {category.skills.map((skill, skillIndex) => {
                  ...
                  return (
                    <div 
                      key={skillIndex}
                      className="flex flex-col items-center p-3 bg-gray-50 rounded-xl transition-all duration-300 hover:bg-violet-50 hover:scale-105 group"
                    >
                      <IconComponent 
                        size={40} 
                        className={`${skill.color} mb-3 group-hover:scale-110 transition-transform duration-300`}
                      />
                      <span className="text-gray-500 text-xs font-medium text-center group-hover:text-violet-700">
                        {skill.name}
                      </span>
                    </div>
                  );
          ...
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {coreSkills.map((skill, index) => (
                <div 
                  key={index}
                  className="p-4 bg-gradient-to-br from-violet-50 to-purple-50 rounded-xl border border-violet-100 transition-all duration-300 hover:shadow-md"
                >
                  <h4 className="font-semibold text-violet-700 mb-1">{skill.name}</h4>
        ...
        <div className="text-center">
          <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 transition-all duration-300 hover:shadow-xl">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {softSkills.map((skill, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="flex flex-col items-center p-4 bg-violet-50 rounded-xl transition-all duration-300 hover:bg-violet-100 hover:scale-105 group"
                  >
                    <IconComponent 
                      size={24} 
                      className="text-violet-700 mb-2" 
                    />
                    <span className="text-violet-700 text-sm font-medium text-center group-hover:text-violet-800">
                      {skill.name}
                    </span>
                  </div>
                );
